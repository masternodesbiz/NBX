...
#if defined(NDEBUG)
#error "Netbox.Wallet cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uNBX) are considered zero fee (for relaying and mining)
...
const std::string strMessageMagic = "Netbox.Wallet Signed Message:\n";
...

    {
        ...
        if (Params().RequireStandard() && !AreInputsStandard(tx, view))
            return state.Invalid(error("AcceptToMemoryPool: nonstandard transaction input"),
                                 REJECT_NONSTANDARD, "nonstandard transaction input");
        ...
        if(nSigOps > nMaxSigOps)
            return state.Invalid(
                    error("AcceptToMemoryPool: too many sigops %s, %d > %d", hash.ToString(), nSigOps, nMaxSigOps),
        ...
        if (fRejectInsaneFee && nFees > ::minRelayTxFee.GetNonZeroFee(nSize) * 10000)
            return state.Invalid(
                    error("AcceptToMemoryPool: insane fees %s, %d > %d",
                          hash.ToString(),
                          nFees, ::minRelayTxFee.GetNonZeroFee(nSize) * 10000),
                    REJECT_INVALID, "insane fee");
...
//        }
        ...
        if (!CheckInputs(tx, state, view, true, flags, true)) {
            return state.Invalid(
                    error("AcceptToMemoryPool: CheckInputs failed %s", hash.ToString()),
                    REJECT_INVALID, "CheckInputs failed");
        }
        ...
        if (!CheckInputs(tx, state, view, true, flags, true)) {
            return state.Invalid(
                    error("AcceptToMemoryPool: BUG! PLEASE REPORT THIS! ConnectInputs failed against MANDATORY but not STANDARD flags %s",
                          hash.ToString()),
                    REJECT_INVALID, "BUG! PLEASE REPORT THIS! ConnectInputs failed against MANDATORY but not STANDARD flags");
        }
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee)
{
...

    {
        ...
        if (!CheckInputs(tx, state, view, false, flags, true)) {
            return error("AcceptableInputs: CheckInputs failed %s", hash.ToString());
        }
        ...
        //        {
        //            return error("AcceptableInputs: : BUG! PLEASE REPORT THIS! CheckInputs failed against MANDATORY but not STANDARD flags %s", hash.ToString());
        //        }
...
{
    return blockValue * 0.5 + 0.5;
}

int64_t GetActivityPayment(int nHeight, int64_t blockValue)
{
    return blockValue * 0.21 + 0.5;
}

int64_t GetTeamPayment(int nHeight, int64_t blockValue)
{
    return blockValue * 0.08 + 0.5;
}
...
bool static FlushStateToDisk(CValidationState& state, FlushStateMode mode)
{
    ...
    try {
        if ((mode == FLUSH_STATE_ALWAYS) ||
            ...
            if (!fPreventBestBlockSaving && mode != FLUSH_STATE_IF_NEEDED) {
...
bool CheckBlockHeader(const CBlockHeader& block, int nHeight, CValidationState& state, bool fCheckPOW)
{
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (!CheckBlockHeader(block, nHeight, state, !IsPoS))
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    // Reject block.nVersion<5 blocks
    if (Params().IsStakeModifierV2(nHeight) ? block.nVersion != 6 : block.nVersion != 5) {
        return state.Invalid(error("%s : rejected nVersion=%i block", __func__, block.nVersion),
...
bool AcceptBlockHeader(const CBlock& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (!CheckBlockHeader(block, pindexPrev ? (pindexPrev->nHeight + 1) : 0, state, false)) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (pindex->nStatus & BLOCK_HAVE_DATA) {
        ...
        // return state.DoS(20, error("AcceptBlock() : already have block %d %s", nHeight, pindex->GetBlockHash().ToString()), REJECT_DUPLICATE, "duplicate");
        LogPrintf("AcceptBlock() : already have block %d %s", nHeight, pindex->GetBlockHash().ToString());
        return true;
    }
    ...
    if (isPoS) {
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                for (const CTxIn& tmpIn : stakeTxIn.vin){
                    if(tmpIn.prevout == in.prevout){
...
std::string GetBlockPosFilename(const CDiskBlockPos& pos, const char* prefix)
{
    return GetDataDirForDb() + "blocks" + (char)boost::filesystem::path::preferred_separator + strprintf("%s%05u.dat", prefix, pos.nFile);
}
...
bool CVerifyDB::VerifyDB(CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    nCheckLevel = std::max(0, std::min(5, nCheckLevel));
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        boost::this_thread::interruption_point();
        uiInterface.ShowProgress(_("Verifying blocks..."), std::max(1, std::min(99, (int)(((double)(chainActive.Height() - pindex->nHeight)) / (double)nCheckDepth * (nCheckLevel >= 5 ? 50 : 100)))));
        ...
        // check for inconsistencies during memory-only disconnect of tip blocks
        if (pindex == pindexState && (coins.GetCacheSize() + pcoinsTip->GetCacheSize()) <= nCoinCacheSize) {
            bool fClean = true;
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        bool fMissingSporks = !pSporkDB->SporkExists(SPORK_4_NEW_PROTOCOL_ENFORCEMENT);
...
int ActiveProtocol()
{
    // SPORK_4 is used for 70913 (v3.1.0+)
    if (IsSporkActive(SPORK_4_NEW_PROTOCOL_ENFORCEMENT))
