...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new NBX address for receiving payments.\n"
            ...
            "\nResult:\n"
            "\"nbxaddress\"        (string) New NBX address\n"
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturns the current NBX address for receiving payments to this account.\n"

            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"

            "\nResult:\n"
            "\"nbxaddress\"       (string) The account NBX address\n"
...
UniValue getreservedaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "getreservedaddress\n"
            "\nReturns a new reserved NBX address.\n"
            "This is for use with raw transactions, NOT normal use.\n"

            "\nResult:\n"
            "{\n"
            "  \"address\": \"address\", (string) New NBX address\n"
            "  \"index\": n              (numeric, optional) Address index in HD wallet.\n"
            "}\n"

            "\nExamples:\n" +
            HelpExampleCli("getreservedaddress", "") +
            HelpExampleRpc("getreservedaddress", ""));
    ...
    if (pwalletMain->mapKeyMetadata.count(keyID)) {
        CKeyMetadata &metadata = pwalletMain->mapKeyMetadata[keyID];
        if (metadata.has_key_origin && !metadata.key_origin.path.empty()) {
            ret.push_back(Pair("index", (int64_t) metadata.key_origin.path[metadata.key_origin.path.size() - 1]));
        }
    }
    return ret;
}
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new NBX address, for receiving change.\n"
            "This is for use with raw transactions, NOT normal use.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "setaccount \"nbxaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"nbxaddress\"      (string, required) NBX address to be associated with an account.\n"
            "2. \"account\"         (string, required) The account to assign the address to.\n"

            "\nExamples:\n" +
            HelpExampleCli("setaccount", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\" \"myaccount\"") +
            HelpExampleRpc("setaccount", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\", \"myaccount\""));
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "getaccount \"nbxaddress\"\n"
            "\nReturns the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"nbxaddress\"      (string, required) NBX address for account lookup.\n"

            "\nResult:\n"
            "\"accountname\"        (string) the account address\n"

            "\nExamples:\n" +
            HelpExampleCli("getaccount", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\"") +
            HelpExampleRpc("getaccount", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\""));
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"nbxaddress\"  (string) NBX address associated with the given account\n"
            "  ,...\n"
            "]\n"
...
void PreSendMoney(const std::vector<std::pair<CScript, CAmount>>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, bool fUseIX = false, bool fAllowZero = false)
{
    CAmount nValue = 0;
    for (const PAIRTYPE(CScript, CAmount) &s : vecSend) {
        if (s.second < 0 || (!fAllowZero && s.second <= 0))
    ...
    if (pwalletMain->IsLocked()) {
        ...
        LogPrintf("PreSendMoney() : %s", strError);
    ...
    if (!pwalletMain->CreateTransaction(vecSend, wtxNew, reservekey, nFeeRequired, strError, NULL, ALL_COINS, fUseIX, (CAmount)0)) {
        ...
        LogPrintf("PreSendMoney() : %s\n", strError);
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
    }
}

void PreSendMoney(const CTxDestination& address, CAmount nValue, CWalletTx& wtxNew, CReserveKey& reservekey, bool fUseIX = false, bool fAllowZero = false)
{
    std::vector<std::pair<CScript, CAmount> > vecSend;
    CScript scriptPubKey = GetScriptForDestination(address);
    vecSend.push_back(std::make_pair(scriptPubKey, nValue));
    return PreSendMoney(vecSend, wtxNew, reservekey, fUseIX, fAllowZero);
}

void SendMoney(const CTxDestination& address, CAmount nValue, CWalletTx& wtxNew, bool fUseIX = false, bool fAllowZero = false)
{
    CReserveKey reservekey(pwalletMain);
    PreSendMoney(address, nValue, wtxNew, reservekey, fUseIX, fAllowZero);
    if (!pwalletMain->CommitTransaction(wtxNew, reservekey, (!fUseIX ? "tx" : "ix")))
        throw JSONRPCError(RPC_WALLET_ERROR, "Error: The transaction was rejected! This might happen if some of the coins in your wallet were already spent, such as if you used a copy of wallet.dat and coins were spent in the copy but not marked as spent here.");
}

UniValue presendtoaddresses(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "presendtoaddresses {\"address\":amount,...}\n"
            "\nPrepare send an amount to a given addresses. The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"amounts\"             (string, required) A json object with addresses and amounts\n"
            "    {\n"
            "      \"address\":amount   (numeric) NBX address is the key, the numeric amount in NBX is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "  \"fee\": x.xxx    (numeric) (numeric) The amount of the fee in NBX.\n"
            "}\n"

            "\nExamples:\n" +
            HelpExampleCli("presendtoaddresses", "\"{\\\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\\\":0.01,\\\"NbMDdS7JjQHKACRtJT3aGm31BLW2eNvALs\\\":0.02}\"") +
            HelpExampleRpc("presendtoaddresses", "{\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\":0.01,\"NbMDdS7JjQHKACRtJT3aGm31BLW2eNvALs\":0.02}"));
    ...
    for (std::vector<std::string>::const_iterator it(addrList.begin()); it != addrList.end(); ++it) {
        CBitcoinAddress address(*it);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid NBX address: ") + *it);

        CScript scriptPubKey = GetScriptForDestination(address.Get());
        CAmount nAmount = AmountFromValue(sendTo[it - addrList.begin()]);
    ...
    CWalletTx wtx;
    CReserveKey reservekey(pwalletMain);
    PreSendMoney(vecSend, wtx, reservekey);

    UniValue res(UniValue::VOBJ);
    std::string strHex = EncodeHexTx(static_cast<CTransaction>(wtx));
    res.push_back(Pair("hex", strHex));
    res.push_back(Pair("fee", ValueFromAmount(wtx.GetValueOut() - wtx.GetDebit())));
    return res;
}

UniValue presendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            "presendtoaddress \"nbxaddress\" amount\n"
            "\nPrepare send an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"nbxaddress\"  (string, required) NBX address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in NBX to send. eg 0.1\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("presendtoaddress", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\" 0.1") +
            HelpExampleRpc("presendtoaddress", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\", 0.1"));
    ...
    CWalletTx wtx;
    CReserveKey reservekey(pwalletMain);
    PreSendMoney(address.Get(), nAmount, wtx, reservekey);
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
                "sendtoaddress \"nbxaddress\" amount ( \"comment\" \"comment-to\" )\n"
                ...
                HelpRequiringPassphrase() + "\n"
                                            ...
                                            "\nExamples:\n" +
                HelpExampleCli("sendtoaddress", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\" 0.1") +
                HelpExampleCli("sendtoaddress", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\" 0.1 \"donation\" \"seans outpost\"") +
                HelpExampleRpc("sendtoaddress", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\", 0.1, \"donation\", \"seans outpost\""));
...
UniValue sendtoaddressix(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            "sendtoaddressix \"nbxaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("sendtoaddressix", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\" 0.1") +
            HelpExampleCli("sendtoaddressix", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\" 0.1 \"donation\" \"seans outpost\"") +
            HelpExampleRpc("sendtoaddressix", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\", 0.1, \"donation\", \"seans outpost\""));
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"nbxaddress\",         (string) NBX address\n"
            "      amount,                 (numeric) The amount in NBX\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            "signmessage \"nbxaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"nbxaddress\"      (string, required) NBX address to use for the private key.\n"
            ...
            "\nAs json rpc\n" +
            HelpExampleRpc("signmessage", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\", \"my message\""));
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress \"nbxaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given nbxaddress in transactions with at least minconf confirmations.\n"

            "\nArguments:\n"
            "1. \"nbxaddress\"      (string, required) NBX address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"

            "\nResult:\n"
            "amount                 (numeric) The total amount in NBX received at this address.\n"

            "\nExamples:\n"
            "\nThe amount from transactions with at least 1 confirmation\n" +
            HelpExampleCli("getreceivedbyaddress", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\"") +
            "\nThe amount including unconfirmed transactions, zero confirmations\n" +
            HelpExampleCli("getreceivedbyaddress", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\" 0") +
            "\nThe amount with at least 6 confirmation, very safe\n" +
            HelpExampleCli("getreceivedbyaddress", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\" 6") +
            "\nAs a json rpc call\n" +
            HelpExampleRpc("getreceivedbyaddress", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\", 6"));
...
UniValue getreceivedbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in NBX received for this account.\n"

            "\nExamples:\n"
            "\nAmount received by the default account with at least 1 confirmation\n" +
            HelpExampleCli("getreceivedbyaccount", "\"\"") +
            "\nAmount received at the \"myaccount\" account including unconfirmed amounts with zero confirmations\n" +
            HelpExampleCli("getreceivedbyaccount", "\"myaccount\" 0") +
            "\nThe amount with at least 6 confirmation, very safe\n" +
...
CAmount GetAccountBalance(CWalletDB& walletdb, const std::string& strAccount, int nMinDepth)
{
    CAmount nBalance = 0;
...
UniValue getbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw std::runtime_error(
            ...
            HelpExampleCli("getbalance", "\"\"") +
            "\nThe total amount in the account named \"myaccount\" with at least 6 confirmations\n" +
            HelpExampleCli("getbalance", "\"myaccount\" 6") +
            "\nAs a json rpc call\n" +
            HelpExampleRpc("getbalance", "\"myaccount\", 6"));
...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw std::runtime_error(
            ...
            "3. amount            (numeric, required) Quantity of NBX to move between accounts.\n"
            ...
            "\nExamples:\n"
            "\nMove 0.01 NBX from the default account to the account named \"myaccount\"\n" +
            HelpExampleCli("move", "\"\" \"myaccount\" 0.01") +
            "\nMove 0.01 NBX from \"timotei\" to \"akiko\" with a comment\n" +
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"tonbxaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a NBX address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"tonbxaddress\"      (string, required) NBX address to send funds to.\n"
            "3. amount                (numeric, required) The amount in NBX. (transaction fee is added on top).\n"
            "4. minconf               (numeric, optional, default=1) Only use funds with at least this many confirmations.\n"
            ...
            "\nExamples:\n"
            "\nSend 0.01 NBX from the default account to the address, must have at least 1 confirmation\n" +
            HelpExampleCli("sendfrom", "\"\" \"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\" 0.01") +
            "\nSend 0.01 from the \"myaccount\" account to the given address, funds must have at least 6 confirmations\n" +
            HelpExampleCli("sendfrom", "\"myaccount\" \"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\" 0.01 6 \"donation\" \"seans outpost\"") +
            "\nAs a json rpc call\n" +
            HelpExampleRpc("sendfrom", "\"myaccount\", \"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\", 0.01, 6, \"donation\", \"seans outpost\""));
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            ...
            HelpExampleCli("sendmany", "\"myaccount\" \"{\\\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\\\":0.01,\\\"NbMDdS7JjQHKACRtJT3aGm31BLW2eNvALs\\\":0.02}\"") +
            "\nSend two amounts to two different addresses setting the confirmation and comment:\n" +
            HelpExampleCli("sendmany", "\"myaccount\" \"{\\\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\\\":0.01,\\\"NbMDdS7JjQHKACRtJT3aGm31BLW2eNvALs\\\":0.02}\" 6 \"testing\"") +
            "\nAs a json rpc call\n" +
            HelpExampleRpc("sendmany", "\"myaccount\", {\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\":0.01,\"NbMDdS7JjQHKACRtJT3aGm31BLW2eNvALs\":0.02}, 6, \"testing\""));
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "Each key is a NBX address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of NBX addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) NBX address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"nbxaddress\"      (string) NBX address associated with the keys.\n"

            "\nExamples:\n"
            "\nAdd a multisig address from 2 addresses\n" +
            HelpExampleCli("addmultisigaddress", "2 \"[\\\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\\\",\\\"NbMDdS7JjQHKACRtJT3aGm31BLW2eNvALs\\\"]\"") +
            "\nAs json rpc call\n" +
            HelpExampleRpc("addmultisigaddress", "2, [\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\",\"NbMDdS7JjQHKACRtJT3aGm31BLW2eNvALs\"]"));
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    bool fIncludeEmpty = ParseBool(params[1]);
...
UniValue listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in NBX received by the address\n"
...
static void MaybePushAddress(UniValue & entry, const char* key, const CTxDestination &dest)
{
...
void ListTransactions(const CWalletTx& wtx, const std::string& strAccount, int nMinDepth, bool fLong, UniValue& ret)
{
    CAmount nFee;
    std::string strSentAccount;
    std::list<COutputEntry> listReceived;
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"address\":\"nbxaddress\",         (string) NBX address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in NBX. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in NBX. This is negative and only available for the \n"
            ...
            "\nList the most recent 10 transactions for the \"myaccount\" account\n" +
            HelpExampleCli("listtransactions", "\"myaccount\"") +
            "\nList transactions 100 to 120 from the \"myaccount\" account\n" +
            HelpExampleCli("listtransactions", "\"myaccount\" 20 100") +
            "\nAs a json rpc call\n" +
            HelpExampleRpc("listtransactions", "\"myaccount\", 20, 100"));
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending NBXs\n"
    ...
    SecureString strWalletPass = SecureStringFromString(params[0].get_str());

    bool stakingOnly = ParseBool(params[2]);
...
UniValue walletpassphrasechange(const UniValue& params, bool fHelp)
{
    ...
    SecureString strOldWalletPass = SecureStringFromString(params[0].get_str());
    SecureString strNewWalletPass = SecureStringFromString(params[1].get_str());
...
UniValue walletlock(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 0))
        throw std::runtime_error(
            ...
            HelpExampleCli("sendtoaddress", "\"NYkDC3hHouRaCSAMNsSxFj5Xv1h5etPzGT\" 1.0") +
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending NBXs\n" +
            HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n" +
            HelpExampleCli("signmessage", "\"nbxaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; Netbox.Wallet server stopping, restart to run with encrypted wallet.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending NBXs.\n"
    ...
    bool fUnlock = ParseBool(params[0]);
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in NBX/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total NBX balance of the wallet\n"
            "  \"unconfirmed_balance\": xxx, (numeric) the total unconfirmed balance of the wallet in NBX\n"
            "  \"immature_balance\": xxxxxx, (numeric) the total immature balance of the wallet in NBX\n"
            "  \"txcount\": xxxxxxx,         (numeric) the total number of transactions in the wallet\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx          (numeric) the transaction fee configuration, set in NBX/kB\n"
            "}\n"
...
UniValue getxpub(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                "getxpub\n"
    ...
    CKeyID seed_id = pwalletMain->GetHDChain().seed_id;
    if (seed_id.IsNull())
        throw JSONRPCError(RPC_WALLET_ERROR, "HD seed is empty");
    CKey seed;
    if (!pwalletMain->GetKey(seed_id, seed))
        throw JSONRPCError(RPC_WALLET_ERROR, "HD seed not found");

    CExtKey masterKey, accountKey;
    masterKey.SetMaster(seed.begin(), seed.size());

    CExtPubKey masterPub = masterKey.Neuter();
    unsigned char data[BIP32_EXTKEY_SIZE];
    masterKey.Encode(data);
    masterPub.Encode(data);
    CBitcoinExtPubKey b58pub;
    b58pub.SetKey(masterPub);
...
UniValue getfirstaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                "getfirstaddress\n"
                ...
                "\nExamples:\n" +
                HelpExampleCli("getfirstaddress", "") +
                HelpExampleRpc("getfirstaddress", ""));

    LOCK2(cs_main, pwalletMain->cs_wallet);

    CKeyID first_address = pwalletMain->GetHDChain().first_address;
    if(!IsMine(*pwalletMain, first_address))
        throw JSONRPCError(RPC_WALLET_ERROR, "First address not found");

    UniValue ret(UniValue::VSTR);
    ret = CBitcoinAddress(first_address).ToString();
    return ret;
}
...
UniValue reservebalance(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 0) {
        bool fReserve = ParseBool(params[0]);
        if (fReserve) {
            if (params.size() == 1)
...
UniValue setstakesplitthreshold(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("setstakesplitthreshold", "5000") +
            HelpExampleRpc("setstakesplitthreshold", "5000"));
...
UniValue getstakesplitthreshold(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("getstakesplitthreshold", "") +
            HelpExampleRpc("getstakesplitthreshold", ""));
...
{
    bool fEnable = ParseBool(params[0]);

    if (fHelp || params.size() < 1 || (fEnable && params.size() != 2) || params.size() > 2)
        throw std::runtime_error(
            "autocombinerewards enable ( threshold )\n"
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same NBX address\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("autocombinerewards", "true 500") +
            HelpExampleRpc("autocombinerewards", "true 500"));
...
UniValue gethdseed(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                "gethdseed\n"
    ...
    try {
        mnemonic = pwalletMain->GetHdSeed();
    } catch (const std::runtime_error &e) {
        throw JSONRPCError(RPC_WALLET_ERROR, e.what());
    }

    UniValue ret(UniValue::VSTR);
    // TODO: return UniValue with SecureString
    ret = mnemonic.c_str();
    return ret;
}
