...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a NBX address (e.g. %1)").arg("NSQfiSgf56pyrpwNaUk1LUYcBv2AZwx1yQ"));
...
{
    // return if URI is not valid or is no NBX: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::NBX, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert nbx:// to nbx:
    //
    //    Cannot handle this later, because nbx:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::NBX, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open nbx.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Netbox.Wallet.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Netbox.Wallet.lnk
...
{
    return GetAutostartDir() / "Netbox.Wallet.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a Netbox.Wallet.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Netbox.Wallet\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Netbox.Wallet app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add Netbox.Wallet app to startup item list
...
{
    if (sizeof(boost::filesystem::path::value_type) == 1)
