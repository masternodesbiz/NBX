...
void ThreadMapPort()
{
    ...
    if (devlist) {
        ...
        if (r) {
            ...
            std::string strDesc = "Netbox.Wallet " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Netbox.Wallet is probably already running."), addrBind.ToString());
        else
...
bool CAddrDB::Read(CAddrMan& addr)
{
    ...
    uint64_t dataSize = boost::filesystem::file_size(pathAddr);
    // Don't try to resize to a negative number if file is small
    if (dataSize >= sizeof(uint256))
        dataSize -= sizeof(uint256);
...
bool CBanDB::Read(banmap_t& banSet)
{
    ...
    // use file size to size memory buffer
    uint64_t dataSize = boost::filesystem::file_size(pathBanlist);
    // Don't try to resize to a negative number if file is small
    if (dataSize >= sizeof(uint256))
        dataSize -= sizeof(uint256);
    std::vector<unsigned char> vchData;
