...
UniValue getpoolinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) NBX address of current masternode\n"
...
UniValue listmasternodes(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw std::runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode NBX address\n"
    ...
    for (PAIRTYPE(int, CMasternode) & s : vMasternodeRanks) {
        ...
        if (mn != NULL) {
            if (strFilter != "" &&
                ...
                mn->GetStatus().find(strFilter) == std::string::npos &&
                CBitcoinAddress(mn->pubKeyCollateralAddress.GetID()).ToString().find(strFilter) == std::string::npos &&
                mn->addr.ToString().find(strFilter) == std::string::npos)
                continue;
...
UniValue getmasternodestatus (const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw std::runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) NBX address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
UniValue getmasternodewinners (const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) NBX MN Address\n"
