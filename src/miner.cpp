...

    {
        ...
        if (!fProofOfStake) {
            ...
            if (txNew.vout.size() > 1) {
                ...
                CAmount blockValue = nFees + GetBlockValue(pindexPrev->nHeight, 0);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet)
{
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("NBXMiner : ProcessNewBlock, block not accepted");
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            if (!chainActive.Tip() || chainActive.Tip()->nHeight < Params().LAST_POW_BLOCK()) {
        ...
        LogPrintf("Running NBXMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("NBXMiner exception");
    } catch (...) {
        LogPrintf("NBXMiner exception");
    }

    LogPrintf("NBXMiner exiting\n");
}
