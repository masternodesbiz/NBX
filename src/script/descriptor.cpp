...
public:
    ...
    bool ToPrivateString(const SigningProvider& arg, std::string& ret) const override
    {
        ...
        ret = CBitcoinSecret(key).ToString();
        return true;
    }
};
...
//    {
//        std::string ret = m_extkey.pubkey.GetHex() + FormatHDKeypath(m_path);
//        if (IsRange()) {
//            ret += "/*";
...
//        out = CBitcoinSecret(key.key).ToString() + FormatHDKeypath(m_path);
//        if (IsRange()) {
//            out += "/*";
...
protected:
    std::string ToStringExtra() const override { return CBitcoinAddress(m_destination).ToString(); }
...
std::unique_ptr<DescriptorImpl> InferScript(const CScript& script, ParseScriptContext ctx, const SigningProvider& provider)
{
    ...
    txnouttype txntype;
    //assert(Solver(script, txntype, data));
