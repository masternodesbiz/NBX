...
Basic code to support P2P connectivity to a nbxd.
...
behavior of a nbxd against 1 or more other nbxd instances, or against
known outcomes, or both.

* Set the ```num_nodes``` variable (defined in ```ComparisonTestFramework```) to start up
1 or more nodes.  If using 1 node, then ```--testbinary``` can be used as a command line
option to change the nbxd binary used by the test.  If using 2 or more nodes,
then ```--refbinary``` can be optionally used to change the nbxd that will be used
on nodes 2 and up.
...
Each ```TestInstance``` consists of:
  - a list of ```[object, outcome, hash]``` entries
    * ```object``` is a ```CBlock```, ```CTransaction```, or
    ...
    complete headers-chains when requested from the nbxd, to allow writing
    tests where blocks can be delivered out of order but still processed by
    headers-first nbxd's.
    * ```outcome``` is ```True```, ```False```, or ```None```.  If ```True```
    or ```False```, the tip is compared with the expected tip -- either the
    block passed in, or the hash specified as the optional 3rd entry.  If
    ```None``` is specified, then the test will compare all the nbxd's
  ...
  - ```sync_every_transaction```: ```True/False```.  Analogous to
    ...
    then the contents of the entire mempool are compared across all nbxd
